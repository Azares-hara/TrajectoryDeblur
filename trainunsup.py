...
#WARNING!WARNING! UNSUPERVISED STATE SWITCH 
import os
import re
import torch
...
class Trainer:
    def __init__(self, args, model, criterion, optimizer):
        ...
        if hasattr(args, 'unpaired_data_root') and args.unpaired_data_root:
            from dataset.gopro_large import GOPRO_Large  
            self.unpaired_loader = DataLoader(
                GOPRO_Large(args, mode='train', unpaired=True),
            ...
            self.unpaired_iter = iter(self.unpaired_loader)
        else:
            self.unpaired_loader = None
            self.unpaired_iter = None
    ...
    def train(self, start_epoch, num_epochs):
        ...
        for epoch in range(start_epoch, num_epochs):
            self.epoch = epoch
            weights = get_disc_weights(epoch)
            self.schedule = {
                'trajectory':    {'active': lambda e: e < 100,  'weight': 0.01},
                'blur_band':     {'active': lambda e: e >= 10, 'weight': 0.01},
                'depth_layer':   {'active': lambda e: e >= 20, 'weight': 0.01},
                'ghost_edge':    {'active': lambda e: e >= 150, 'weight': 0.02}, 
                'circular':      {'active': lambda e: e >= 150, 'weight': 0.02},  
                'edge_map':      {'active': lambda e: e >= 130, 'weight': 0.05},
                'adversarial':   {'active': lambda e: e >= 150, 'weight': 0.02},
            }          
            #freeze schedule            
            generator = self.model._inner('G') if hasattr(self.model, '_inner') else self.model
            if epoch < 30:
            #phase1:Only trajectory_head
                for param in generator.parameters():
                    param.requires_grad = False
                for param in generator.trajectory_head.parameters():
                    param.requires_grad = True
            elif epoch < 50:
               #phase 2:Trajectory + decoder
                for param in generator.parameters():
                    param.requires_grad = False
                for param in generator.trajectory_head.parameters():
                    param.requires_grad = True
            ...
            with tqdm(total=len(self.loaders['train_blur']), ncols=100,
                ...
                for idx in range(len(self.real_loader)):
                    if epoch >= 100 and self.unpaired_loader is not None:
                        try:
                            batch = next(self.unpaired_iter)
                        except StopIteration:
                            self.unpaired_iter = iter(self.unpaired_loader)
                            batch = next(self.unpaired_iter)
                        real_sharp = None 
                        ...
                        try:
                            ...
                            batch = next(self.real_iter)
                        real_sharp = batch.get("sharp", None)
                    ...
                    if trajectory is not None and epoch < 100:
                        dx_dy = trajectory[:, :2]    
                        conf  = trajectory[:, 2:3]
                        # smoothness
                        ...
                        # alignment (only where motion blur is predicted)
                    ...
                    color_variance = fake_for_loss.std(dim=1).mean()  # std across RGB channels
                    color_penalty = - 0.01 * color_variance  # punish channel disagreement
                    ...
                    #discriminator
                    #adversarial training is reserved for a dedicated fine-tuning
                    ...
                    total_loss = total_loss + color_penalty 
                    total_loss = total_loss + traj_smooth_loss + traj_align_loss 
                    #Warmup anchor
                    if epoch < 30:
                        warmup_weight = max(0.0, 1.0 - epoch / 30.0)
                        identity_loss = F.l1_loss(fake_for_loss, input_for_loss)
                        total_loss    = total_loss + warmup_weight * identity_loss

                    if edge_map is not None and epoch >= 130:
                        total_loss = total_loss + W_EDGE_CONS * edge_loss
                    if sharp_for_loss is not None:
                        total_loss = total_loss + W_L1* l1_loss
                        total_loss = total_loss + W_LPIPS * lpips_loss
                        ...
                        total_loss = total_loss + 1.0 * warp_loss
                    ...
                    for idx, batch in enumerate(self.loaders['val']):
                        ...
                        if idx < 3:
                            save_tensor_as_image(
                                triplet[0],
                                os.path.join(
                                    self.save_dir, f"val_epoch{epoch+1}_{base_name}.png"
                                )
                            )
                        else:
                            break
